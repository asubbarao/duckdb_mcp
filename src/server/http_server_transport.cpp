...
void SetupRoutes(ServerType &server, const HTTPServerConfig &config,
	...
	if (!config.cors_origins.empty()) {
		server.Options(".*", [&config](const CPPHTTPLIB_NAMESPACE::Request &req, CPPHTTPLIB_NAMESPACE::Response &res) {
			...
			if (!cors_value.empty()) {
				...
				res.set_header("Access-Control-Allow-Headers", "Content-Type, Authorization, Accept, MCP-Protocol-Version, Mcp-Method, Mcp-Name");
	...
	auto mcp_handler = [&config, &request_handler](const CPPHTTPLIB_NAMESPACE::Request &req,
		...
		if (!Accepts(req.get_header_value("Accept"), "application/json") ||
		    !Accepts(req.get_header_value("Accept"), "text/event-stream")) {
			RPCError(res, 406, -32600, "Accept must include application/json and text/event-stream");
			return;
		}
		...
		if (!yyjson_is_obj(root) || JSONUtils::GetString(root, "jsonrpc") != "2.0" ||
		    ...
		    (id && !yyjson_is_str(id) && !yyjson_is_int(id)) ||
		    yyjson_obj_get(root, "result") || yyjson_obj_get(root, "error")) {
		...
		auto meta = yyjson_obj_get(params, "_meta");
		...
		if (req.get_header_value_count("MCP-Protocol-Version") > 1 ||
		    (modern && id && (version.empty() || version != body_version))) {
		...
		try {
			...
			if (response.size() > config.max_response_bytes) {
				RPCError(res, 500, -32000, "Response exceeds configured byte limit; execution may have completed; do not replay writes", id);
				return;
			}
